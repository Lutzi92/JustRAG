...
- `query_cache_*`, `step_back_*`, `mmr_*`, `rerank_blend_alpha*`, `top_n_*` — retrieval pipeline (`query_cache_similarity_threshold_*` for per-query-type thresholds). `rerank_candidate_depth` (+ `_lookup`/`_enumeration`/`_complex_reasoning`, per-KB) sizes the pre-rerank candidate pool; sentinel `0` = legacy `max(4×top-N, 50)` with a reranker / `max(2×top-N, 30)` without, per-route inherits the global key, floor = top-N, cap 500. `EffectiveRerankDepth` in `internal/vector/rerank_depth.go`; logged as `rerank_depth` on `rag.search.stages`. Registry `Min` is 0 but the value parser only accepts 0 or 10–500, so 1–9 silently falls back to the legacy formula.
