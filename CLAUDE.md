...
- `bm25_simple_arm_enabled` — second unstemmed tsvector arm (recovers terms the stemmer destroys); `bm25_tiered_boost_enabled` (default off, retirement candidate — see below) — coarse ts_rank×100/×10 IDF proxy. `bm25_scoring_mode` (`ts_rank` | `bm25`, default `ts_rank`, per-KB) selects the keyword arm's ranking formula: `ts_rank` is Postgres's built-in two-argument `ts_rank()` (term frequency only, no IDF, default normalisation 0 — no document-length or cover-density/proximity weighting); `bm25` computes real Okapi BM25 (`bm25_k1` default 1.2 [0.5–3.0], `bm25_b` default 0.75 [0–1], both per-KB) from per-KB/per-dimension stats tables `bm25_kb_stats_<dim>`/`bm25_term_stats_<dim>`, which the worker's `bm25_stats_refresh` maintenance loop recomputes every 15 min (stale = no stats row, or a stats row older than the newest chunk, or older than 24h; skips KBs with an active ingestion). A KB/dimension with no stats row falls back to `ts_rank` per query and increments `rag_bm25_mode_fallback_total{reason}`; `rag_keyword_arm_mode_total{mode}` counts which mode actually ran. **Default stays `ts_rank`** — A/B on `production-ppm-2026-08` (`docs/retrieval.md` "Keyword arm scoring: ts_rank vs BM25") showed `bm25` lifting lookup recall/MRR but regressing `complex_reasoning` MRR ~7 pp (RRF weights are tuned for `ts_rank`'s score scale); `bm25` ships as an opt-in per-KB mode pending a Wave-3 `rrf_weight_bm25`/α retune. Operator note: bump `queryCacheSchemaVersion` (or clear the query cache) whenever `bm25_scoring_mode` is flipped in production — the mode is deliberately not hashed into the query-cache shape. See the "Real BM25 keyword arm" recipe in `docs/feature-recipes.md`.
