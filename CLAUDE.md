...
- `tabular_profile_*` — sheet profiler (Phase 1 of the 2026-09-04 spreadsheet rework; LLM column descriptions; no effect until Phase 2 wires ingest)
...
| Retrieval pipeline | `vector`, `ai`, `splitter`, `processor`, `parser`, `pptx`, `sheetsource`, `tabular/profile` | Search service (BM25 + vector + reranker + MMR), embedder/grader/refiner LLM calls, chunking, document parsing, KG **write** path (`processor/kg_store.go`); `sheetsource` = streaming spreadsheet reader (xlsx/xls/ods/csv, typed cells + styles + merges + validations), `tabular/profile` = sheet profiler (regions, header blocks, column roles, sheet kind) — Phase 1 of the 2026-09-04 spreadsheet rework; **not yet wired into ingest** |
