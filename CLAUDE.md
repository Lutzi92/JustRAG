...
- `chat_condense_keep_raw_enabled` (default **off**, per-KB) — when a follow-up is condensed, the verbatim utterance is additionally searched as one extra RRF list on both arms (`SearchOptions.RawQuery`); standard + Supervisor paths only (not public API / OpenAI-compat / MCP server / agent teams / plan-execute / agentic / DRIFT). No LLM call, one extra embedding + one extra BM25 query per follow-up turn. Part of the query-cache shape hash. Flip on once `eval/golden/multi-turn-de.jsonl` (Wave 2) scores it. `chat_longcontext_top_k` (default 200 = the former hardcoded constant, range 50–500, global-only) sizes the long-context chunk pool. `eval_regression_recall_pp` (default 2.0) / `eval_regression_mrr_pp` (default 3.0) — percentage-point thresholds for the scheduled-eval regression gate (global-only, no registry entry; set from the admin Agent panel's "Geplante Evaluationen" section).
