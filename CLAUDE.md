...
| Retrieval pipeline | `vector`, `ai`, `splitter`, `processor`, `parser`, `pptx`, `sheetsource`, `tabular`, `tabular/profile`, `tabular/render`, `tabular/ingest` | Search service (BM25 + vector + reranker + MMR), embedder/grader/refiner LLM calls, chunking, document parsing, KG **write** path (`processor/kg_store.go`); the spreadsheet cluster (2026-09-04 rework, wired into ingest since Phase 2) — `sheetsource` = streaming spreadsheet reader (xlsx/xls/ods/csv, typed cells + styles + merges + validations), `tabular/profile` = sheet profiler (regions, header blocks, column roles, sheet kind), `tabular` = typed materialiser (staged COPY into native Postgres tables + `tabular_column_values` value index) + catalog, `tabular/render` = key:value hybrid text renderer, `tabular/ingest` = the one-file orchestrator (`parser.SpreadsheetParser` and the processor's spreadsheet branch both drive it) |
...
| Tabular Q&A (table_query) | `chat_tabular_query_enabled` (+ `tabular_profile_*`, `tabular_max_rows`, `tabular_embed_max_rows`, `tabular_column_values_max_distinct`, `chat_tabular_charts_enabled`) — **needs OPERATOR PREREQUISITE grants** | 0048, 0069 | Structured spreadsheet Q&A |
