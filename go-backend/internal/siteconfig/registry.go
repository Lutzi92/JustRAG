...
var kbConfigRegistry = []KBConfigField{
	...
	{Key: "chat_longcontext_mode", Type: FieldEnum, Group: "Orchestrator", Label: "Long-context: Verarbeitung", Help: "Wie der breite Chunk-Pool (bis zu 200 Chunks) zur Antwort wird. flat = der komplette Pool geht roh an das Antwortmodell (bisheriges Verhalten). map_reduce = pro Chunk-Gruppe erst ein günstiger Extraktionsaufruf (Aussage + wörtliches Zitat je Quelle), das Antwortmodell sieht dann nur diese Befunde statt der Rohtexte — deutlich kürzerer Antwort-Prompt und weniger Positionsbias, dafür ein zusätzlicher Fast-Tier-Aufruf je Gruppe. Wirkt nur, wenn Long-Context aktiv ist. Standard: flat.", Enum: []string{"flat", "map_reduce"}},
