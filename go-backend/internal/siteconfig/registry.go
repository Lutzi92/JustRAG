...
var kbConfigRegistry = []KBConfigField{
	...
	{Key: "rerank_candidate_depth", Type: FieldInt, Group: "Retrieval", Label: "Reranker-Kandidatentiefe", Help: "Wie viele fusionierte Kandidaten pro Suche an den Reranker gehen. 0 = bisheriges Verhalten (4×Top-N, mindestens 50). Mehr Kandidaten = mehr Recall auf komplexen Fragen, aber mehr Reranker-Latenz (jina-v3: grob 100–200 ms je 30 Kandidaten).", Min: f(0), Max: f(500)},
	{Key: "rerank_candidate_depth_lookup", Type: FieldInt, Group: "Retrieval", Label: "Kandidatentiefe (lookup)", Help: "Pro-Route-Override; 0 erbt den globalen Wert.", Min: f(0), Max: f(500)},
	{Key: "rerank_candidate_depth_enumeration", Type: FieldInt, Group: "Retrieval", Label: "Kandidatentiefe (enumeration)", Help: "Pro-Route-Override; 0 erbt den globalen Wert.", Min: f(0), Max: f(500)},
	{Key: "rerank_candidate_depth_complex_reasoning", Type: FieldInt, Group: "Retrieval", Label: "Kandidatentiefe (complex)", Help: "Pro-Route-Override; 0 erbt den globalen Wert.", Min: f(0), Max: f(500)},
	...
	{Key: "bm25_tiered_boost_enabled", Type: FieldBool, Group: "Retrieval", Label: "BM25 tiered boost", Help: "Veraltet — im 2026-09 A/B auf allen Routen negativ; Kandidat für Entfernung. Standardmäßig aus."},
	{Key: "bm25_scoring_mode", Type: FieldEnum, Group: "Retrieval", Label: "BM25-Modus", Help: "ts_rank = bisheriges Postgres-Ranking (nur Termfrequenz, kein IDF). bm25 = echtes BM25 (IDF + Längennormalisierung aus den bm25_*_stats-Tabellen, die der Worker alle 15 min nachführt). Fällt pro Anfrage auf ts_rank zurück, solange eine KB noch keine Statistik hat. Nach dem Umschalten query_cache_schema_version anheben.", Enum: []string{"ts_rank", "bm25"}},
	{Key: "bm25_k1", Type: FieldFloat, Group: "Retrieval", Label: "BM25 k1", Help: "Termfrequenz-Sättigung im echten BM25-Modus. Standard 1,2.", Min: f(0.5), Max: f(3.0)},
	{Key: "bm25_b", Type: FieldFloat, Group: "Retrieval", Label: "BM25 b", Help: "Längennormalisierung im echten BM25-Modus (0 = keine, 1 = voll). Standard 0,75.", Min: f(0), Max: f(1)},
	...
	{Key: "chat_condense_keep_raw_enabled", Type: FieldBool, Group: "Retrieval", Label: "Rewrite ⊕ Rohfrage", Help: "Bei kondensierten Folgefragen wird zusätzlich die wörtliche Nutzerfrage als eigene RRF-Liste gesucht (Vektor + BM25). Kein Modellaufruf, eine Einbettung + eine Stichwortsuche mehr pro Folgefrage. Standardmäßig aus."},
	...
	{Key: "chat_longcontext_mode", Type: FieldEnum, Group: "Orchestrator", Label: "Long-context: Verarbeitung", Help: "Wie der breite Chunk-Pool (bis zu 200 Chunks) zur Antwort wird. map_reduce (Standard seit Wave 5) = pro Chunk-Gruppe erst ein günstiger Extraktionsaufruf (Aussage + wörtliches Zitat je Quelle), das Antwortmodell sieht dann nur diese Befunde statt der Rohtexte — deutlich kürzerer Antwort-Prompt und weniger Positionsbias, dafür ein zusätzlicher Fast-Tier-Aufruf je Gruppe. flat = der komplette Pool geht roh an das Antwortmodell (Verhalten vor Wave 3). Gemessen (W5-R1, 24 Fragen): map_reduce gewinnt 34 von 36 entscheidbaren Richtervergleichen (Wilson-Untergrenze 0,819), Abdeckung +5,0 Prozentpunkte, Laufzeit 1,28x. Wirkt nur, wenn Long-Context aktiv ist. Standard: map_reduce; ein nicht erkannter Wert fällt auf flat zurück.", Enum: []string{"flat", "map_reduce"}},
	...
	{Key: "citation_validation_enabled", Type: FieldBool, Group: "Prüfung", Label: "Zitatprüfung", Help: "Prüft jede Quellenangabe zuerst deterministisch gegen den zitierten Text (Wortfolgen-Abgleich, kein Modellaufruf). Bleibt ein Zitat dabei unbelegt, folgt ein Ähnlichkeitsvergleich per Einbettung: bis zu zwei Einbettungs-Aufrufe je unbelegtem Zitat (Antwortsatz + zitierter Abschnitt, innerhalb einer Antwort zwischengespeichert). Das ist ein Modell-API-Aufruf, aber kein LLM-Antwortaufruf; er ist ab Werk aktiv (Schwelle citation_validation_semantic_threshold, Standard 0,85; 0 schaltet ihn ab). Markiert unbelegte Zitate; ein Verdachtsfall löst die vertiefte Aussagenprüfung bzw. Self-RAG aus — die Selbstprüfung wird ausschließlich hierüber angestoßen. Standardmäßig aktiv."},
	{Key: "chat_citation_spans_enabled", Type: FieldBool, Group: "Prüfung", Label: "Zitat-Fundstellen", Help: "Zitiert der Antworttext [N], extrahiert ein Fast-Tier-Modell nach der Antwort ein wörtliches Zitat aus der Quelle, das exakt gegen den Chunk geprüft wird; die Fundstelle wird im Quellen-Popover hervorgehoben. Ein zusätzlicher Modellaufruf pro Antwort. Standardmäßig aus."},
	{Key: "chat_citation_spans_max_sources", Type: FieldInt, Group: "Prüfung", Label: "Zitat-Fundstellen: max. Quellen", Help: "Obergrenze für unterschiedliche Quellen, die pro Antwort in einem Aufruf an die Fundstellen-Extraktion gehen. Standard 12, gültiger Bereich 1–50.", Min: f(1), Max: f(50)},
	{Key: "chat_citation_spans_timeout_ms", Type: FieldInt, Group: "Prüfung", Label: "Zitat-Fundstellen: Timeout (ms)", Help: "Zeitbudget für den Fundstellen-Modellaufruf, danach bleibt die bisherige Zitatprüfung unverändert bestehen. Standard 8000, gültiger Bereich 1000–60000.", Min: f(1000), Max: f(60000)},
	{Key: "chat_conflict_surfacing_enabled", Type: FieldBool, Group: "Prüfung", Label: "Widersprüchliche Quellen erkennen", Help: "Vergleicht die für eine Antwort gefundenen Quellen in einem zusätzlichen Fast-Tier-Aufruf darauf, ob sie sich widersprechen oder ob eine Quelle eine andere ersetzt (neuere Fassung). Gefundene Widersprüche werden der Antwort als Vorgabe mitgegeben, auf der Nachricht gespeichert und im Chat als Hinweis angezeigt. Ein zusätzlicher Modellaufruf pro Antwort; läuft nur, wenn mindestens zwei verschiedene Dateien im Kontext stehen. Standardmäßig aus."},
	{Key: "chat_conflict_max_chunks", Type: FieldInt, Group: "Prüfung", Label: "Widersprüche: max. Quellen", Help: "Obergrenze für Quellen, die pro Antwort verglichen werden (die am besten bewerteten zuerst). Standard 12, gültiger Bereich 2–30.", Min: f(2), Max: f(30)},
	{Key: "chat_conflict_timeout_ms", Type: FieldInt, Group: "Prüfung", Label: "Widersprüche: Timeout (ms)", Help: "Zeitbudget für den Vergleichsaufruf; danach wird die Antwort ohne Widerspruchshinweis erzeugt. Standard 6000, gültiger Bereich 1000–30000.", Min: f(1000), Max: f(30000)},
	...
	{Key: "chat_tabular_router_enabled", Type: FieldBool, Group: "Tabular", Label: "Tabular router", Help: "Deterministic pre-pass that turns table questions into validated read-only SQL before falling back to normal retrieval; kill switch effective only together with chat_tabular_query_enabled (default on)."},
	{Key: "chat_tabular_router_model", Type: FieldString, Group: "Tabular", Label: "Tabular router model", Help: "Fast-tier model for the tabular router's SQL generation and repair; empty falls through to model_tier_fast."},
	{Key: "chat_tabular_router_max_rows", Type: FieldInt, Group: "Tabular", Label: "Tabular router max rows", Help: "Row cap on the router-generated SQL query's result set.", Min: f(10), Max: f(1000)},
	{Key: "chat_tabular_router_max_repairs", Type: FieldInt, Group: "Tabular", Label: "Tabular router max repairs", Help: "Number of LLM repair attempts allowed when the generated SQL fails validation or execution.", Min: f(0), Max: f(5)},
	{Key: "chat_tabular_router_timeout_ms", Type: FieldInt, Group: "Tabular", Label: "Tabular router timeout (ms)", Help: "Wall-clock budget for the router's SQL generation and execution before it gives up and falls back to normal retrieval.", Min: f(500), Max: f(30_000)},
	{Key: "chat_tabular_router_schema_max_tokens", Type: FieldInt, Group: "Tabular", Label: "Tabular router schema budget (tokens)", Help: "Token budget for the table-schema description injected into the router's SQL-generation prompt.", Min: f(1000), Max: f(60_000)},
	{Key: "chat_tabular_guidance_max_tokens", Type: FieldInt, Group: "Tabular", Label: "Tabular guidance budget (tokens)", Help: "Token budget for the per-KB catalog summary folded into the answer system prompt (the tabular guidance block). Separate from the tabular router's own schema budget above.", Min: f(1000), Max: f(30_000)},
	{Key: "tabular_max_file_bytes", Type: FieldInt, Group: "Tabular", Label: "Max spreadsheet upload size (bytes)", Help: "Uploaded spreadsheets above this size are rejected with HTTP 413. Does not apply to other file types.", Min: f(1_048_576), Max: f(2_147_483_647)},
	{Key: "tabular_large_file_bytes", Type: FieldInt, Group: "Tabular", Label: "Large-spreadsheet threshold (bytes)", Help: "Ingested spreadsheets above this size are treated as 'large' for materializer concurrency (tabular_large_file_concurrency).", Min: f(1_048_576), Max: f(1_073_741_824)},
	{Key: "tabular_large_file_concurrency", Type: FieldInt, Group: "Tabular", Label: "Large-spreadsheet materializer concurrency", Help: "How many 'large' spreadsheets (tabular_large_file_bytes) may be materialized concurrently.", Min: f(1), Max: f(8)},
