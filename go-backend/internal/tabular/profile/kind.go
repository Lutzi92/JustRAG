...
// filled and whose row has <= 2 other filled cells.
func labelValuePairRatio(s *sheetsource.Sample, reg Region) float64 {
	...
	for r := reg.Top; r <= reg.Bottom; r++ {
		...
		for c := reg.Left; c <= reg.Right; c++ {
			...
			filled++
			if cell.Kind != sheetsource.KindText || rowFilled > 3 {
				continue
			}
...
// low). Preflight ruling R3 (single-kept-column table gate) is enforced via
...
func classifyKind(s *sheetsource.Sample, reg Region, hb *headerBlock, ok bool) SheetKind {
	...
	if pairs >= formPairRatio && (merges >= formMinMerges || (ok && headerContrast(s, reg, hb) < formHeaderContrast)) {
		return KindForm
	}
	if ok {
		...
		if dataRows >= tableMinDataRows || (reg.OpenEnded && dataRows >= 1) {
			...
			if cells > 0 && float64(fill)/float64(cells) >= tableMinFill &&
