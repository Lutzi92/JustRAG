...
type AggregateMetrics struct {
	...
	// MeanLatencyMs is the mean of QuestionReport.LatencyMs across the same
	// non-errored question set the retrieval metrics above are computed
	// over (W6-R7 cost measurement). Not a pointer: 0 is a valid latency for
	// a synthetic/instant adapter and every existing report already carries
	// a LatencyMs on each question, so backfilling this field never turns a
	// "no data" state into a misleading 0 — see report.go's comment on why
	// on-disk reports are never regenerated by this change and therefore
	// need no migration.
	MeanLatencyMs float64 `json:"mean_latency_ms,omitempty"`
	// MeanLLMCalls is the mean of AgentTrace.LLMCalls across questions that
	// carry a non-nil Agent (W6-R7). A pointer, nil when no question in the
	// aggregate carried an Agent trace at all — distinct from "traced but
	// zero calls" — so a pre-Wave-6 report, or a run against an adapter that
	// never dispatches through an orchestrator, has this field omitted
	// rather than present-and-zero.
	MeanLLMCalls *float64 `json:"mean_llm_calls,omitempty"`
}
