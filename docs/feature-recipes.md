...
Cost when on: one extra fast-tier call over ≤ `chat_conflict_max_chunks` sources, **+631 ms / +268 ms** per turn across two runs at the default cap. Retrieval is untouched — the on/off runs are identical to three decimals on recall, precision, MRR and nDCG, which is the direct demonstration that the pass is strictly post-retrieval.
...
2. **Operator SQL (not measured on dev, W6-R8a)** — the dev fixture runs answer tools off and the eval harness never exercises the answer-tools loop, so there is no eval-harness before/after number for this key. Verify a change in production instead:
   ...
   GROUP BY mode;
   ```
   run once before and once after flipping the key, per KB, and compare the tool-call distribution.
