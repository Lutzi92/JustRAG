...
**Side finding (historical):** the coarse ts_rank×100/×10 IDF proxy behind the former `bm25_tiered_boost_enabled` key was net negative under `ts_rank` on every route on this fixture (overall MRR −3.5 pp vs. the `ts_rank`/no-boost baseline, more than the 1.3 pp overall-MRR noise band) and didn't clearly help under `bm25` either (real BM25 already carries an IDF term, so the boost stacks rather than substitutes). The key was deprecated in Wave 3 and **removed in Wave 7** (migration 0074 deletes any stored row); the A/B that retired it is the Wave-2 grid in `docs/retrieval.md` §"Keyword arm scoring: ts_rank vs BM25 (2026-09)" (cells B and D — the only run in which the boost was ever on), not the Wave-3 retune record, which ran with the boost off throughout.

## Per-query-type cache thresholds (T0-4)
...
Pure-config (no migration, no LLM). Sentinel `0` inherits the global threshold.
