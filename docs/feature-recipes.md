...
Cost when on: one extra fast-tier call over ≤ `chat_conflict_max_chunks` sources. **Wave 5** measured **+631 ms / +268 ms** per turn across two runs at the default cap; **Wave 6**'s re-measurement (same default cap, the rewritten CERT pair questions above) found **+585.1 ms** (recall 0.700 `on` vs 0.708 `off` — see `eval/golden/cert-recency-de.acceptance.md`, the section immediately above this one). Both waves agree retrieval is untouched — the on/off runs are identical to three decimals on recall, precision, MRR and nDCG, which is the direct demonstration that the pass is strictly post-retrieval.
