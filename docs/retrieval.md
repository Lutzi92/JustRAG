...
**Tiered boost interaction (historical)**: cells B and D above used `bm25_tiered_boost_enabled`, a coarse CASE-based ts_rank×100/×10 multiplier that approximated "rare term → bigger boost" without real corpus statistics. With real BM25 scoring already carrying an IDF term, the boost stacked on top rather than substituting for it — cell D (bm25 + boost) didn't clearly beat cell C (bm25, no boost) on lookup MRR (0.913 vs 0.895) or complex_reasoning (both regress by the same ~7pp), and cell B (ts_rank + boost) regressed on every route relative to A (ts_rank, no boost) by more than the noise band, including a −3.5pp overall MRR regression.

**The key was deprecated in Wave 3 and removed in Wave 7.** The measurement that retired it is the **Wave-2 A/B in the grid above** (cells B and D against A and C, on `eval/golden/production-ppm-2026-08.jsonl`) — that is the only run in which the boost was ever ON, and it is recorded here rather than in a separate acceptance file. The Wave-3 retune grid in `eval/golden/bm25-retune.acceptance.md` is **not** the retiring measurement: every cell in it ran with `--bm25-tiered-boost off`; it is cited below only for the `bm25`-mode weight/α question. There is no flag, no `buildBoostExpr` and no `--bm25-tiered-boost` eval override any more, and migration 0074 deletes any stored `site_configs` / `kb_site_configs` row. The B and D cells above cannot be reproduced on the current code; A and C can.
...
--bm25-tiered-boost off --refresh-bm25-stats`; that override no longer exists —
the key was removed in Wave 7). 25–29 of 89 questions routed
