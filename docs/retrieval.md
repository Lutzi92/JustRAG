...
The feature stays **off**: both pre-stated gates failed (0 of 8 CERT pairs flagged — a fixture property, since MMR never assembles both halves of the queried pair; 0.124 false-positive flag rate on the PPM set against a ≤ 0.10 bar, and that rate is an **upper bound** pending a detector fix). Mechanism, wire shape and the full numbers: `docs/agent-orchestration.md` and `docs/feature-recipes.md` §"Conflict / supersession surfacing"; record in `eval/golden/cert-recency-de.acceptance.md`.
