...
**The key was deprecated in Wave 3 and removed in Wave 7**; the Wave-3 A/B that retired it is in `eval/golden/bm25-retune.acceptance.md`. There is no flag, no `buildBoostExpr` and no `--bm25-tiered-boost` eval override any more, and migration 0074 deletes any stored `site_configs` / `kb_site_configs` row. The B and D cells above cannot be reproduced on the current code; A and C can.
