...
**Golden set:** 89 active questions across three routes (lookup 43, enumeration 14, complex_reasoning 32). `global_synthesis` is not represented in this fixture. 11 unanswerable questions are commented out in `production.jsonl` pending a loader schema extension (they can't currently be loaded because they have empty `must_cite_file_ids`).
...
cd go-backend && go build -o ./cmd/eval/eval ./cmd/eval
```
...
./cmd/eval/eval --golden ../eval/golden/production.jsonl --top-k 10 \
...
The `cmd/eval --production-context` path was spot-checked against the HTTP chat endpoint on 5 stratified questions picked deterministically from `production.jsonl`:
