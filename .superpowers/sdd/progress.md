...
=== FEATURE: Agent Teams PHASE 2 — team eval + hardening (2026-07-03) ===
Plan: docs/superpowers/plans/2026-07-03-agent-teams-phase2.md
Spec: docs/superpowers/specs/2026-07-03-agent-teams-phase2-design.md
Mode: subagent-driven, main, BUILD-ONLY (no commits; user commits). Baseline diffs via scratchpad/sdd-pkg.sh (snapshot|diff N <paths>).
Tasks: 1 mig 0063 | 2 served tool catalog | 3 corpus gate reorder | 4 telemetry ids | 5 msg attribution BE | 6 eval TeamDispatchAdapter+--team-id | 7 admineval team runs | 8 trajectory grouping FE | 9 attribution chip FE | 10 verify
P2 Task 1: COMPLETE (review Approved, independently re-verified vs live DB). migrations/main/0063_team_attribution_eval.sql applied (v63): messages.team_id/agent_id + eval_runs.team_id, all SET NULL FKs, idempotent.
  Minor (final triage): no indexes on the new columns (consistent w/ 0061 chats columns; revisit if aggregation queries land).
P2 Task 2: COMPLETE (review Approved). GetRegistry → {fields, tools} from the SAME AvailableTools closure as save validation (verified both call sites); FE consumes served tools, NON_PRIVILEGED_TOOLS deleted (zero refs). 5/5 BE tests, 249 FE tests, tsc/lint clean. No Important findings.
P2 Task 3: COMPLETE (review Approved, truth-table verified). Corpus-table gate reorder in http_send.go 509-532: runCompare → willRunTeam → willRunCorpusTable with !runCompare && !willRunTeam leading; suppression ifs deleted; all 7 conjuncts preserved once; downstream gates + log untouched. Classifier/LLM-confirm now short-circuits on comparison/team turns. Full chat suite green.
P2 Task 4: COMPLETE (review Approved). DecisionRecorder gains teamID/agentID *string (appended); recordAgentDecision forwards; deep-chat site derives from teamSel, standard site nil/nil; adminagentmetrics INSERT $8/$9 order-verified; routes.go decisionRecorderAdapter pass-through (compile-required, judged in-scope). New post_metrics_test.go w/ race-safe fake. build/test/vet clean.
  Minors (final triage): forwarding test lacks agentID-populated case; the teamSel-derivation branch at the call site itself untested (matches persistChatSelection precedent).
P2 Task 5: COMPLETE (review Approved, no findings). messages attribution BE: MessageRow/messageDBRow teamId/agentId (omitempty verified by test), AddMessage $10/$11, ALL messages SELECTs incl. GetMessageAncestors recursive CTE arm (implementer caught what the brief's grep would miss), single hoisted derivation feeding AddMessage + recordAgentDecision. build/test/vet clean.
P2 Task 6: COMPLETE (review Approved — opus). internal/eval/team_adapter.go(+test) + cmd/eval --team-id. Projection verified byte-identical to orchestrator adapter; hard-error semantics STRUCTURAL (no prod field → cannot fall back); per-KB team cache; --team-id w/o --production-context fatal; --team-id beats --orchestrator-dispatch=false (coherent). types.go untouched (byte-stable). Real names: Question.KbID/.Language; ContentsForQuestion(qID,k)(contents,fileNames,ok). 3 tests + full eval suite green.
  INTERPRETATION NOTE (for user): a team whose router selects nobody (ErrTeamNoRoute) shows as per-question ERRORS in the eval report, not zero-recall rows — correct per spec, but reports read differently.
P2 Task 7: COMPLETE (review Approved — opus, after controller-directed judge fix). admineval team runs: TeamID on BOTH CreateRun + kbCreateRunRequest (KB tab posts to /api/kb/{id}/eval/runs — brief missed it, implementer caught); validate-before-insert (400 no-kb / 400 not-attached); eval.Run team_id across Insert+Get+List; RunDeps.AgentTeams wired + nil-guard; FE team select + 2 keys. JUDGE FIX: ctxCache + ChatContextForQuestion on team adapter (mirrors production adapter), runner types adapter as chatContextProvider (compiler-enforced, orchestrator adapter unaffected via dynamic agentTracer assertion); CLI judge upgrades automatically. All suites green.
  Minors (final triage): admin-scope form keeps stale team selection across KB-A→KB-B switch (backend 400 guards it; one-line reset suggested); store_pg.go minmax modernization hint.
P2 Task 8: COMPLETE (review Approved after fix wave). TrajectoryPanel grouped team view: renderRow() extraction (flat path verified byte-identical line-by-line), renderTeamGrouped (router header + per-agent hop groups + synthesis row), LABELS de+en. Fix wave (empirically re-verified on 3 edge shapes): totalRows excludes borderless group headers — last-row border invariant holds. 5/5 tests, 253 suite green, tsc/lint clean.
  Minors (final triage): zero-hop/unmatched-hop cases handled but untested; nesting assertions text-level only; jsdom serializes border 'none' as 'medium' (style asserts should check not-dashed).
P2 Task 9: COMPLETE (review Approved, no Important findings). Attribution chip: Message.teamId/agentId, useChat mapping (single construction site verified), useChatStream stamps committed AI message post-remap (Standard turns stamp null — no staleness), chip in MessageBubble §8 footer (memo + stable useCallback verified; lookup miss renders nothing; ComparisonView unaffected — optional prop). 254 tests, tsc/lint/build clean.
  Minor (final triage): redundant role==='ai' re-checks inside already-gated block.
P2 Task 10: COMPLETE (verification). GATES: full go test ./... green, FE 254 tests/tsc/lint/build green, internal/eval/types.go byte-stable (0 changes). LIVE SMOKE (server :3111, minted JWT): registry {30 fields, 11 tools sorted, no privileged leak — table_query/memory_* now selectable}; chat w/ teamId → chats.team_id persisted, fail-soft cascade visible (team→standard→recency listing→clean LLM error); messages.team_id row not produced locally (turn dies at LLM before AI save — JSON contract unit-tested, threading code-reviewed; live confirm = user acceptance run); eval run POST w/ team_id → 201 queued, eval_runs.team_id persisted; not-attached negative covered by unit tests (single-KB dev DB). Smoke data cleaned (0 rows).
P2 FINAL WHOLE-BRANCH REVIEW (fable): "With fixes" — 0 Critical, 2 Important, both FIXED + 2 fix-before-commit minors FIXED (single fix wave, all controller-verified in source):
  - IMPORTANT #1 FIXED: attribution was derived from teamSel!=nil → comparison/Enhance turns falsely team-attributed (violated DecisionRecorder's own doc). Now attributionIDs(willRunTeam, teamSel) helper, gated + table-tested (incl. the previously-untested agentID branch).
  - IMPORTANT #2 FIXED: eval TeamDispatchAdapter now wires the REAL per-agent SearcherForAgent overlay (mirrors http_send.go:656-670; agent overrides layer over global config in eval — no kb overlay in eval path by pre-existing convention); misattributed comment corrected; regression test added. Team evals now measure production-equivalent retrieval for config-carrying agents.
  - MINOR FIXED: AdminEvalTab resets selectedTeamId on KB switch; cmd/eval fatal-guards --team-id + --trajectory.
  Verify after wave: go build/test/vet/gofmt clean; tsc/254 tests/lint clean.
  Follow-ups queued (not blockers): RunSummary.team_id (runs list can't distinguish team runs), SSE-carried authoritative attribution (FE stamp is optimistic until reload), messages FK index if team deletes get hot, TS AgentConfigField.requiresReingest.

=== AGENT TEAMS PHASE 2 COMPLETE (build-only, ALL UNCOMMITTED; user commits) ===
NEW: migrations/main/0063_team_attribution_eval.sql; internal/eval/team_adapter.go(+test); internal/chat/{post_metrics_test.go,types_attribution_test.go}; web TrajectoryPanel.team.test.tsx
MOD BE: internal/agentteams/handler(+test); internal/chat/{http.go,http_send.go,http_send_helpers.go,post_metrics.go,types.go,store_pg.go}; internal/adminagentmetrics/handler(+test); internal/admineval/{types,handler,kb_handler}(+tests); internal/eval/{store_pg,runner_inproc}.go; internal/app/{routes,worker}.go; cmd/eval/main.go
MOD FE: agents/{api.ts,AgentsView(+test)}; admin/AdminEvalTab.tsx; TrajectoryPanel.tsx; MessageBubble.tsx; ChatView.tsx; hooks/{useChat,useChatStream,useKbAgents}.ts; types.ts; translations.ts (+2 keys)
REMAINING: user commits; USER ACCEPTANCE = first real `--team-id` eval run (or eval-tab team run) against a reachable LLM — doubles as the live validation of router→specialists→synthesis AND messages.team_id population. Interpretation note: teams whose router selects nobody show as per-question ERRORS in reports (by design).
